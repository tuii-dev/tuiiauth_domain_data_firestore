...
import 'package:tuiiauth_domain_data_firestore/src/i18n/auth_i18n.dart';
...
class FirebaseAuthDataSourceImpl implements FirebaseAuthDataSource {
  ...
  Future<bool> validateSignUpHash(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Failure(message: 'Hash validation failed!'.i18n);
      }
    } on Exception {
      throw Failure(message: 'Hash validation failed!'.i18n);
    }
  ...
  Future<UserModel> createUserWithEmailAndPassword({
    ...
    try {
      ...
      throw Failure(
          message: 'Authentication failed: Unanticipated error'.i18n,
          code: "UNANTICIPATED");
    }
  }
  ...
  Future<UserModel> loginWithEmailAndPassword(
    ...
    try {
      ...
      if (hint is! PhoneMultiFactorInfo) {
        ...
        throw Failure(
          message: 'Requires multifactor auth'.i18n,
  ...
  Future<void> completeMultiFactorLogin(
    ...
    if (_mfaStreamController != null) {
      try {
        await firebaseAuth.verifyPhoneNumber(
            ...
            verificationFailed: (FirebaseAuthException e) {
              _mfaStreamController!.sink.add(MfaResponseModel(
                  ...
                  payload: e.message ??
                      'Failed to obtain multifactor verification code.'.i18n));
            },
        ...
        throw Failure(
            message: 'Failed to obtain multifactor verification code.'.i18n,
  ...
  Future<bool> enrollUserMultiFactorAuth(
    ...
    try {
      ...
      throw Failure(
          message: 'Failed to enroll user.  Message: '.i18n + (e.message ?? ''),
  ...
  Future<bool> unenrollUserMultiFactorAuth() async {
    try {
      ...
      throw Failure(
          message:
              'Failed to unenroll user.  Message: '.i18n + (e.message ?? ''),
  ...
  Future<bool> resolveMultiFactorSignIn(
    ...
    try {
      ...
      throw Failure(
          message: 'Login failed.  Message: '.i18n + (e.message ?? ''),
  ...
  Future<CreateZoomTokenResponse> getZoomToken(String zoomTokenUrl,
    ...
    if (response.statusCode == 200) {
      ...
      throw Failure(message: 'Failed to create zoom token.'.i18n);
    }
  }
  ...
  Future<bool> revokeZoomToken(String zoomTokenUrl, String tutorId,
    ...
    if (response.statusCode == 200) {
      ...
      throw Failure(message: 'Failed to revoke zoom token.'.i18n);
    }
  }
  ...
  Future<bool> sendPhoneVerificationSms(
    ...
    if (response.statusCode != 200) {
      throw Failure(message: 'Failed to send phone verification sms.'.i18n);
    }
  ...
  Future<bool> verifyPhone(String firebaseToken, String uid, String phoneNumber,
    ...
    if (response.statusCode != 200) {
      throw Failure(message: 'Invalid code specified.'.i18n);
    }
    ...
    try {
      ...
      if (!user.isEmpty && (user.id != null && user.id!.isNotEmpty)) {
        ...
        if (user.isStripeSetUpComplete == true) {
          final profileCompletion = user.onboardingState?.profileCompletion;
          if (profileCompletion != null && profileCompletion.payments != true) {
            ...
            if (user.onboardingState?.profileCompletion != null) {
              ...
              profileCompletion = ProfileCompletionModel.fromUser(user);
              profileCompletion = profileCompletion.copyWith(payments: true);
            }
            final onboardingState = user.onboardingState != null
                ? user.onboardingState!
                    .copyWith(profileCompletion: profileCompletion)
